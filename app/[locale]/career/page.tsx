...
import { Benefits, Jobopening, Recruitments, Culture, Departments, Worktype} from "@/data/site";
...
export default function CareersPage() {
  ...
  const filtered = Jobopening.filter((job) => {
    const matchSearch =
      job.key.toLowerCase().includes(searchQuery.toLowerCase()) ||
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-10">
            ...
            >
              {Departments.map((d, index) => (
                <option key={index} value={d}>{t(`departments.${d}`)} </option>
              ))}
            </select>
            <select
              value={selectedType}
            ...
            >
              {Worktype.map((type, index) => (
                <option key={index} value={type}>{t(`workTypes.${type}`)} </option>
              ))}
            </select>
          </div>
