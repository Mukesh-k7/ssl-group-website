        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {services.map((service) => (
              <div
                key={service.title}
                className="rounded-xl border border-white/10 bg-gunmetal/30 p-6"
              >
                <service.icon className="mb-4 h-8 w-8 text-industrial-blue" />
                <h3 className="font-heading font-semibold text-white">{service.title}</h3>
                <p className="mt-2 text-sm text-metallic/80">{service.description}</p>
              </div>
            ))}
          </div>
          <div className="mt-16 rounded-xl border border-white/10 bg-charcoal/50 p-8">
            <h3 className="font-heading text-xl font-bold text-white">Supported Incoterms</h3>
            <div className="mt-4 flex flex-wrap gap-3">
              {incoterms.map((term) => (
                <span
                  key={term}
                  className="rounded-lg border border-industrial-blue/30 bg-industrial-blue/10 px-4 py-2 font-mono text-sm text-metallic"
                >
                  {term}
                </span>
              ))}
